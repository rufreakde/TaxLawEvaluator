...
**TaxLawEvaluator** is a UI application for simulating and evaluating tax laws. Users define economy parameters, citizen scenarios (with varying wealth levels), and tax laws as a node graph. The system computes outputs (state income, disposable income per scenario) and scores results via a points system.

Core domain concepts:
- **Economy Inputs** — macro-level economic parameters
- **Scenario Inputs** — citizen profiles with wealth/income distributions
- **Laws as Nodes** — tax rules modeled as a composable node graph
- **Outputs** — state income and available money per scenario
- **Points System** — evaluation metric scoring the law configurations against goals
...
> No build system or source code exists yet. Update this section once the stack is initialized.

Once a framework is set up, typical commands will be added here (e.g., `npm install`, `npm run dev`, `npm test`).
...
- **Deduction Logic**: 
  - Each `TaxNode` present in the graph: `-1` point.
  - Each `FormulaRule` within a node: `-1` point.
...
- **Source Nodes**: Map to `income` or `economy` YAML data.
- **Logic Nodes**: Apply formulas (e.g., `$a * 0.75`).
- **Sink Nodes**: Aggregate results for State Income and Scenario Totals.
